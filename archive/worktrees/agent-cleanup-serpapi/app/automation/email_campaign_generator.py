...
class EmailCampaignGenerator:
    ...
    async def _fetch_market_intelligence(self, keywords: list[str]) -> dict[str, Any]:
        """Fetch market data using alternative methods (legacy SerpAPI removed)"""
        logger.info(f"Using mock market research data for {len(keywords)} keywords")

        # Create mock market results (SerpAPI legacy code removed)
        market_results = {
            "performance_metrics": {"total_queries": len(keywords), "successful_queries": len(keywords)},
            "market_intelligence": {
                "content_themes": ["workflow automation", "cost savings", "efficiency tools"],
                "market_sentiment": {"positive": 0.8, "neutral": 0.15, "negative": 0.05}
            },
            "raw_search_results": [{"query": kw, "results": f"Mock results for {kw}"} for kw in keywords]
        }

        logger.info(f"Mock market research completed: {market_results['performance_metrics']}")
