...
class RallyAIOrchestrator:
    ...
    async def coordinate_market_analysis(
        ...
        agent_tasks = await self._decompose_analysis_query(
            analysis_query, target_revenue_impact, urgency
        )
    ...
    async def _decompose_analysis_query(
        self, query: str, revenue_impact: float, urgency: TaskPriority
    ) -> list[AgentTask]:
    ...
    async def _execute_coordinated_tasks(
        ...
        for task in sorted_tasks:
            ...
            try:
                ...
                logger.info(
                    f"✅ Completed {task.role.value} with confidence {result.confidence_score:.2f}"
                )
    ...
    async def _market_research_agent(self, task: AgentTask) -> dict[str, Any]:
        ...
        analysis = await self.rag_engine.analyze_market_opportunity(
            task.query, use_agent=True, sources=task.context.get("data_sources", [])
        )
    ...
    async def _synthesize_agent_results(
        self, results: list[AgentResult], original_query: str
    ) -> dict[str, Any]:
    ...
    def _assess_revenue_impact(self, results: list[AgentResult]) -> dict[str, Any]:
        ...
        revenue_results = [
            r for r in results if r.agent_role == AgentRole.REVENUE_OPTIMIZER
        ]
    ...
    async def _update_orchestration_metrics(
        self, task_results: list[AgentResult], final_analysis: dict[str, Any]
    ) -> None:
        ...
        self.orchestration_state.update(
            {
                ...
                "average_confidence": (
                    sum(r.confidence_score for r in successful_tasks)
                    / len(successful_tasks)
                    if successful_tasks
                    else 0.0
                ),
                "revenue_impact_score": final_analysis.get(
...
async def main():
    ...
    try:
        analysis = await orchestrator.coordinate_market_analysis(
            test_query, target_revenue_impact=300.0, urgency=TaskPriority.HIGH
        )
