...
class DevelopmentCostEstimator:
    ...
    def estimate_development_costs(self) -> dict:
        ...
        for component in self.components:
            ...
            component_estimates.append({
                ...
                "cost_breakdown": {
                    ...
                    "savings_from_ai": component.estimated_hours * 0.3 * component.hourly_rate  # 30% time savings
                }
            })
        ...
        return {
            ...
            "total_estimates": {
                ...
                "ai_cost_percentage": (total_ai_cost / (total_development_cost + total_ai_cost)) * 100
            },
    ...
    def calculate_ai_assistance_cost(self, ai_hours: int, complexity: str) -> float:
        ...
        sonnet_cost = (sonnet_tokens / 2 / 1_000_000 * self.pricing["sonnet"]["input"]) + \
    ...
    def generate_week1_cost_projections(self, target_reduction_pct: float = 0.30) -> dict:
        ...
        actual_reduction = (baseline_weekly_total - optimized_weekly_total) / baseline_weekly_total

        # Development costs for Week 1
        week1_dev_priority = ["Stripe Integration", "Trial & Conversion Flow"]
        week1_dev_cost = sum(
            estimate["total_cost"] for estimate in self.estimate_development_costs()["component_estimates"]
            if estimate["name"] in week1_dev_priority
        ) / 4  # Spread over 4 weeks
        ...
        return {
            ...
            "revenue_alignment": {
                ...
                "profit_margin": ((400 * 7) - (optimized_weekly_total + week1_dev_cost)) / (400 * 7) * 100,
                "break_even_daily_revenue": (optimized_weekly_total + week1_dev_cost) / 7
            }
        }
    ...
    def create_token_usage_report(self) -> dict:
        ...
        total_ai_hours = sum(comp["ai_assistance_hours"] for comp in dev_estimates["component_estimates"])
        ...
        model_breakdown["sonnet"]["tokens"] = int(total_estimated_tokens * self.ai_distribution.sonnet_pct)
        model_breakdown["haiku"]["tokens"] = int(total_estimated_tokens * self.ai_distribution.haiku_pct)
        model_breakdown["opus"]["tokens"] = int(total_estimated_tokens * self.ai_distribution.opus_pct)
        ...
        for model in model_breakdown:
            ...
            model_breakdown[model]["tasks"] = len(list(getattr(self.ai_distribution, f"{model}_tasks", [])))
        ...
        return {
            ...
            "cost_optimization": {
                "vs_all_opus": {
                    "all_opus_cost": total_estimated_tokens / 1_000_000 * 45,  # Average Opus cost
                    "optimized_cost": total_cost,
                    "savings": (total_estimated_tokens / 1_000_000 * 45) - total_cost,
                    "savings_percentage": ((total_estimated_tokens / 1_000_000 * 45) - total_cost) / (total_estimated_tokens / 1_000_000 * 45) * 100
                },
...
def main():
    ...
    print(f"Total Project Cost: ${dev_estimates['total_estimates']['total_project_cost']:,.0f}")
    print(f"AI Assistance Cost: ${dev_estimates['total_estimates']['total_ai_cost']:.0f}")
    print(f"Development Hours: {dev_estimates['total_estimates']['total_development_hours']}")
    ...
    print("\n🤖 TOKEN USAGE:")
    print(f"Total Tokens: {token_report['token_usage_summary']['total_estimated_tokens']:,}")
    print(f"Total Cost: ${token_report['token_usage_summary']['total_estimated_cost']:.2f}")
    print(f"Sonnet: {token_report['model_distribution']['sonnet_4']['percentage']:.0f}%")
