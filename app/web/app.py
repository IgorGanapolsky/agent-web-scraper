...
ALL_ATTEMPTS_FAILED_MSG = "All attempts with BeautifulSoup failed, falling back to UndetectedChromeScraper"
...
init_file_path = os.path.join(project_root, 'app', '__init__.py')
...
def get_undetected_scraper_class():
    ...
    try:
        ...
        logging.info("Successfully imported UndetectedChromeScraper using absolute import")
        return Scraper
    except ImportError:
        ...
        try:
            ...
            logging.info("Successfully imported UndetectedChromeScraper using relative import")
            return Scraper
        except ImportError:
            logging.error(
...
def _extract_headers_from_soup(soup, tags_to_analyze):
    ...
    if META_DESCRIPTION in tags_to_analyze:
        ...
        if meta_desc and meta_desc.get('content'):
            ...
            logging.info(f"Found {META_DESCRIPTION}: {results[META_DESCRIPTION][:50]}...")
        else:
...
def extract_headers_with_soup(url, tags_to_analyze, retry_count=2, use_undetected_fallback=True):
    ...
    for attempt in range(retry_count + 1):
        try:
            ...
            logging.warning(f"Request error in extract_headers_with_soup for {url}: {str(e)}")
            ...
            logging.error(f"Unexpected error in extract_headers_with_soup for {url}: {str(e)}")
            if attempt == retry_count:  # Last attempt failed
                if use_undetected_fallback:
                    logging.info(ALL_ATTEMPTS_FAILED_MSG)
                    return extract_headers_with_undetected_chrome(url, tags_to_analyze)
    ...
    if use_undetected_fallback:
        logging.info(ALL_ATTEMPTS_FAILED_MSG)
        return extract_headers_with_undetected_chrome(url, tags_to_analyze)
    return {tag: "Error: Failed to fetch data after multiple attempts" for tag in tags_to_analyze}
...
def _extract_headers_from_soup_undetected(soup, tags_to_analyze):
    ...
    for tag in (t for t in tags_to_analyze 
               if t not in ('Title', META_DESCRIPTION)):
        elements = soup.find_all(tag.lower())
        if elements:
            results[tag] = ' | '.join(
                e.get_text(strip=True) for e in elements 
...
@log_exceptions
def extract_headers_with_undetected_chrome(url, tags_to_analyze, max_retries=3):
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            logging.info("Successfully extracted %d items with advanced method", 
                        len(results))
            return results
...
def _create_error_result(url: str, max_retries: int, error: str = None) -> dict:
...
def _make_http_request(url: str) -> tuple[requests.Response | None, str | None]:
...
def _process_single_url(url, progress, results, current_url_info=None, progress_details=None):
        ...
        if result:
            ...
            if progress_details:
                if result.get('Status', '').startswith('✅'):
                    progress_details.success(f"Successfully scraped: {result.get('Title', 'Unknown')}")
                else:
                    progress_details.warning(f"Issue with: {url} - {result.get('Status', 'Unknown')}")
    ...
    try:
        with st.spinner("Searching and scraping..."):
            ...
            search_results = _process_search_results(
                ...
                st.session_state.get('num_results', 10)
            )
            ...
            with progress_col:
                progress_bar = st.progress(0, text=f"Scraping 0 of {total_results} pages...")
            ...
            for i, url in enumerate(urls, 1):
                ...
                progress_bar.progress(progress, text=f"Scraping {i} of {total_results} pages...")
                # Pass the progress placeholders to show real-time updates in main window
                _process_single_url(url, progress, results, current_url_info, progress_details)
        ...
        if 'results_df' in st.session_state and not st.session_state['results_df'].empty:
            try:
                ...
                st.download_button(
                    ...
                    data=buffer,
                    file_name=f"scraped_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
...
def show_export_options():
    ...
    if 'results_df' in st.session_state and not st.session_state['results_df'].empty:
        try:
            ...
            buffer = BytesIO()
            with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
                st.session_state['results_df'].to_excel(writer, index=False, sheet_name='Scraped Data')
                ...
                for i, col in enumerate(st.session_state['results_df'].columns):
                    max_len = max(st.session_state['results_df'][col].astype(str).apply(len).max(), len(col))
...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.get('is_processing', False):
            ...
            if st.button(STOP_SCRAPING_TEXT, type="primary", use_container_width=True, key="stop_button"):
                st.session_state.should_stop = True
                st.session_state.is_processing = False
                st.rerun()
        else:
            ...
            with st.form(key='search_form'):
                st.markdown("<h5 style='margin-top:0.5rem;'>Search term:</h5>", unsafe_allow_html=True)
                ...
                st.markdown("<h5 style='margin-top:1.5rem;'>Number of results to scrape:</h5>", unsafe_allow_html=True)
                ...
                st.markdown("<h5 style='margin-top:1.5rem;'>Headers to extract</h5>", unsafe_allow_html=True)
                col1, col2 = st.columns(2)
                headers_to_extract = []
                with col1:
                    if st.checkbox("Title", value=("Title" in st.session_state.get('selected_headers', ['Title', 'H1', 'H2']))):
                        headers_to_extract.append("Title")
                    if st.checkbox("H1", value=("H1" in st.session_state.get('selected_headers', ['Title', 'H1', 'H2']))):
                        headers_to_extract.append("H1")
                    if st.checkbox("H2", value=("H2" in st.session_state.get('selected_headers', ['Title', 'H1', 'H2']))):
                        headers_to_extract.append("H2")
                    if st.checkbox("H3", value=("H3" in st.session_state.get('selected_headers', []))):
                        headers_to_extract.append("H3")
                with col2:
                    if st.checkbox("H4", value=("H4" in st.session_state.get('selected_headers', []))):
                        headers_to_extract.append("H4")
                    if st.checkbox("H5", value=("H5" in st.session_state.get('selected_headers', []))):
                        headers_to_extract.append("H5")
                    if st.checkbox("H6", value=("H6" in st.session_state.get('selected_headers', []))):
                        headers_to_extract.append("H6")
                    if st.checkbox(META_DESCRIPTION, value=(META_DESCRIPTION in st.session_state.get('selected_headers', []))):
                        headers_to_extract.append(META_DESCRIPTION)
                st.session_state['selected_headers'] = headers_to_extract

                st.markdown("<h5 style='margin-top:1.5rem;'>Additional headers (comma separated):</h5>", unsafe_allow_html=True)
                ...
                submit_clicked = st.form_submit_button("🔍 Start Scraping", use_container_width=True, type="primary")
            ...
            """ % ('true' if st.session_state.get('is_processing', False) else 'false'), unsafe_allow_html=True)
    ...
    if 'results_df' in st.session_state and not st.session_state['results_df'].empty:
        ...
        success_count = len(df[df['Status'].astype(str).str.contains('Success|✅') & ~df['Title'].astype(str).str.startswith('Error')])
        error_count = len(df[df['Status'].astype(str).str.contains('Error|❌') | df['Title'].astype(str).str.startswith('Error')])
