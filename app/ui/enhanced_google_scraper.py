    ...
    try:
        ...
        if 'Meta Description' in tags_to_analyze:
            ...
            if meta_desc and meta_desc.get('content'):
                ...
                logging.info(f"Found Meta Description: {results['Meta Description'][:50]}...")
            else:
...
def search_and_scrape():
    ...
    try:
        with open('scraper.log', 'w') as f:
            f.write(f"Starting new scraping session for: {st.session_state['search_term']}\n")
        logging.info(f"Starting new scraping session for: {st.session_state['search_term']}")
    except Exception as e:
    ...
    try:
        ...
        selected_headers = list(st.session_state['selected_headers'])  # Convert set to list if needed

        # Add any additional headers
        if st.session_state['additional_headers']:
            additional = [h.strip() for h in st.session_state['additional_headers'].split(',')]
        ...
        try:
            ...
            if "organic_results" in results and results["organic_results"]:
                urls_to_scrape = [result["link"] for result in results["organic_results"] if "link" in result]
                ...
                status_container.info(f"Found {len(urls_to_scrape)} URLs to scrape. Starting scraping process...")
                logging.info(f"URLs to scrape: {urls_to_scrape}")
            else:
                status_container.warning("No search results found. Please try a different search term.")
                return
        except Exception as e:
...
def main():
    ...
    with st.sidebar:
        ...
        # Get the search term
        search_term = st.text_input("Enter search term:", value=st.session_state.get('search_term', ''),
                                    key="search_term_input", on_change=handle_enter_key)
        st.session_state['search_term'] = search_term

        st.session_state['num_results'] = st.slider("Number of results to scrape:", min_value=1, max_value=50,
        ...
        with col2:
            ...
            if st.checkbox("Meta Description",
                           value=("Meta Description" in st.session_state.get('selected_headers', []))):
