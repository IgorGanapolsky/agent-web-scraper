...
class TestScraper:
    ...
    async def test_scrape_with_valid_url(self, scraper, mock_session):
        ...
        mock_response = AsyncMock()
        mock_response.text.return_value = "<html><h1>Test Job</h1><div class=\"company\">Test Co</div></html>"
        mock_response.status = 200
        mock_response.headers = {}
    ...
    async def test_save_results(self, scraper, tmp_path, sample_job_data):
        ...
        with patch('aiofiles.open', mock_aioopen), \
            ...
            mock_makedirs.assert_called_once_with(os.path.dirname(os.path.abspath(str(test_file))), exist_ok=True)
